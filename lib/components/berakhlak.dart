...
class Berakhlak extends StatefulWidget {
  const Berakhlak({Key? key}) : super(key: key);

  @override
  State<Berakhlak> createState() => _BerakhlakState();
}

class _BerakhlakState extends State<Berakhlak> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchBerakhlak() async {
    try {
      final response = await http.get(Uri.parse('$_baseUrl/api/profile/berakhlak'));
      ...
      if (response.statusCode == 200) {
        ...
        final imagePath = data['berakhlak_image'] ?? 
                         data['image'] ?? 
                         data['imageUrl'] ?? 
                         data['url'] ?? 
                         data['foto'] ?? 
